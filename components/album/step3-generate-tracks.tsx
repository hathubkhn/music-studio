...
export function AlbumGenerate({ data, onChange, onBack, onFinish }: Props) {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {/* Success banner */}
      {allDone && albumId && (
        <div className="rounded-xl border border-teal-500/30 bg-teal-500/10 p-4 flex items-center gap-4">
          ...
          <Button
            variant="outline"
            size="sm"
            className="border-teal-500/40 text-teal-300 hover:bg-teal-500/10 gap-1.5 shrink-0"
            onClick={() => { setCountdown(null); onFinish(albumId) }}
          >
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="flex items-center gap-3">
          {!isStarted ? (
            ...
            albumId && (
              <Button variant="gradient" onClick={() => { setCountdown(null); onFinish(albumId) }} className="gap-2 px-6">
                <Package className="w-4 h-4" />
                Open Album
                {countdown !== null && <span className="ml-1 text-xs opacity-70">({countdown}s)</span>}
